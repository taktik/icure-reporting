...
const extractKeys = function(dataset: Array<any>, prefix: string = ''): {spec: any, convDataset: Array<any>} {
	return dataset.reduce(({ spec, convDataset }: {spec: any, convDataset: Array<any>}, r) => {
		const row = Object.keys(r).reduce((row: any, k: string) => {
			...
			if (isArray(val) && val.length) {
				...
				if (!grouper) {
					...
					const { spec: subSpec, convDataset: [subRow] } = extractKeys([obj], `${prefixK}_`)
					Object.assign(row, subRow)
					Object.assign(spec, subSpec)
				}
			} else if (isObject(val)) {
				const { spec: subSpec, convDataset: [subRow] } = extractKeys([val], `${prefixK}_`)
				Object.assign(row, subRow)
				Object.assign(spec, subSpec)
			} else {
				row[prefixK] = val
...
export function writeExcel(dataset: Array<any>, filePath: string) {
